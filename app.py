...
class Score(db.Model):
    ...
    puzzle_id = db.Column(db.Integer, db.ForeignKey(
        'puzzle.id'), nullable=False)
...
def home():
    return jsonify({
        ...
        'status': 'running',
        'endpoints': ['/api/health', '/api/daily-puzzle', '/api/register', '/api/login']
    })
...
# Create tables when app starts (Flask 3.0 compatible)
